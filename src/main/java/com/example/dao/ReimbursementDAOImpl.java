...
public class ReimbursementDAOImpl implements ReimbursementDAO {
	public final static Logger log = Logger.getLogger(ReimbursementDAOImpl.class);
	...
	public Reimbursement viewReimbursement(int reimb_id) {
		...
		try (Connection con = dc.getDBConnection()) {
			...
			if (!rs.first()) {
				...
				do {
					thisReimb = new Reimbursement(rs.getInt(1), rs.getInt(2), rs.getTimestamp(3), rs.getTimestamp(4),
	...
	public boolean submitTicket(int amount, String description, int userID, byte[] receipt, String type) {
		...
		try (Connection con = dc.getDBConnection()) {
			int typeAsInt = 0;
			if (type.equalsIgnoreCase("Lodging") || type.equalsIgnoreCase("Travel") || type.equalsIgnoreCase("Food")
				...
				String sql = "INSERT INTO ERS_REIMBURSEMENT (REIMB_AMOUNT, "
						+ "REIMB_DESCRIPTION, REIMB_AUTHOR, REIMB_RECEIPT, REIMB_TYPE_ID) " + "VALUES (?, ?, ?, ?, ?)";
				PreparedStatement ps = con.prepareStatement(sql);
				ps.setInt(1, amount);
	...
	public List<Reimbursement> viewPastTicketsAllUsers() {
		...
		try (Connection con = dc.getDBConnection()) {
			...
			do {
				Reimbursement thisReimb = new Reimbursement(rs.getInt(1), rs.getInt(2), rs.getTimestamp(3),
						rs.getTimestamp(4), rs.getString(5), rs.getBytes(6), rs.getInt(7), rs.getInt(8), rs.getInt(9),
						rs.getInt(10));
	...
	public boolean approveRequest(int resolverID, int requestID) {
		try (Connection con = dc.getDBConnection()) {
			...
			if (viewReimbursement(requestID).getAuthor() == resolverID) {
				log.error("The request was not approved because a finance manager cannot approve their own requests.");
				return false;
			}
			...
			log.info("The request has been approved");
